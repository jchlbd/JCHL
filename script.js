...
function buildQuickCats() {
  els.quickCats.innerHTML = DEPARTMENTS.map(d => {
    // icon — path হলে img tag, emoji হলে text
    const isPath = d.icon && (d.icon.includes('/') || d.icon.endsWith('.png') || d.icon.endsWith('.svg'));
    const iconHTML = isPath
      ? `<img src="${d.icon}" alt="${d.name}" width="18" height="18" style="object-fit:contain;vertical-align:middle;margin-right:2px;filter:invert(20%) sepia(80%) saturate(500%) hue-rotate(190deg)" onerror="this.style.display='none'">`
      : `<span style="font-size:15px;line-height:1">${d.icon}</span>`;
    return `<button class="cat-chip" data-dept="${d.id}">${iconHTML} ${d.name}</button>`;
  }).join('');
